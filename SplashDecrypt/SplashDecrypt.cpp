...
struct Blowfish {
    ...
    Blowfish(const uint8_t* key, int keyLen) {
		const uint32_t sbox0[] = {
	...
	void encrypt(uint8_t* buf, size_t len) {
		for (int i = 0; i < len; i += 8) {
			uint32_t* L = (uint32_t*)&buf[i];
			uint32_t* R = (uint32_t*)&buf[i + 4];
			encrypt(L, R);
		}
	}
...
int main()
{
    std::cout << "Splash Golf Decrypter" << std::endl;
    std::cout << "Written by Ash Wolf, 2023" << std::endl;
    std::cout << std::endl;

    FILE* f = fopen("Splash.bin", "rb");
    if (!f) {
        std::cout << "Failed to open Splash.bin - is this the right directory?" << std::endl;
    ...
    uint8_t block1[256];
    uint8_t block2[256];
    uint8_t* rest = new uint8_t[totalSize - 512];

    fread(block1, 1, 256, f);
    fread(block2, 1, 256, f);
    fread(rest, 1, totalSize - 512, f);

    fclose(f);

    // Decrypt block 2
	Blowfish ctx1((const uint8_t *) "luiabyhcljwdhlaijflnaivure", 26);

	uint8_t keyBuf[32];
	memcpy(keyBuf, "luiabyhcljwdhlaijflnaivure\0\0\0\0\0\0", 32);
	ctx1.encrypt(keyBuf, 32);

	Blowfish ctx2(keyBuf, 32);
	ctx2.decrypt(block2, 256);

    // Write out the decrypted file
    f = fopen("Splash.exe", "wb");
    if (!f) {
        std::cout << "Failed to open Splash.exe for writing - oops" << std::endl;
		std::cout << "Press Return to exit" << std::endl;
        std::getchar();
        return 1;
    }

    fwrite(block2, 1, 256, f);
    fwrite(block1, 1, 256, f);
    fwrite(rest, 1, totalSize - 512, f);

    fclose(f);

    delete[] rest;

    std::cout << "Successfully unpacked Splash.exe!" << std::endl;
    std::cout << "Press Return to exit" << std::endl;
    std::getchar();
    return 0;
}
